...
```tsx
<button className="bg-primary text-on-primary rounded-lg">Click me</button>
```

### Step 7: Dark Mode Just Works™
...
/* → background-color: #d0bcff (in dark mode) */
```
...
✅ **CSS-first Tailwind v4 integration** - Uses `@theme` directive  
✅ **Semantic color utilities** - `bg-primary` instead of `bg-purple-600`  
✅ **Material Design 3 compliant** - All 18 color roles mapped  
✅ **Automatic dark mode** - No JavaScript, pure CSS  
✅ **Simple user setup** - Single CSS import  
✅ **Type-safe utilities** - Tailwind IntelliSense works  
✅ **Future-proof** - Easy to extend with more tokens
...
- ❌ Build-time only (no runtime changes)
- ❌ Doesn't work well with component libraries
- ❌ Users must extend their config
...
- ✅ Pure CSS (no JS config needed)
- ✅ Runtime theme switching possible
- ✅ Works out of the box
- ✅ Users just import CSS
...
```tsx
<button className="bg-primary">{/* Automatically light in light mode, dark in dark mode */}</button>
```
...
├── index.cjs             (CJS code)
├── index.d.ts            (Types)
└── styles.css            (CSS with @import) ✨ NEW
```
...
export default function Page() {
  return (
    <div className="bg-surface p-4">
      <h1 className="text-on-surface text-4xl">Welcome</h1>

      <button className="bg-primary text-on-primary rounded-lg px-4 py-2">Click me</button>

      {/* Or use our Button component */}
      <Button variant="filled">Click me too</Button>
    </div>
  );
}
```
...
- ✅ Material Design 3 semantic utilities
- ✅ Automatic dark mode support
- ✅ Simple user setup (one CSS import)
- ✅ Type-safe utilities (Tailwind IntelliSense)

Users can now use Tailwind utilities with Material Design 3 semantics! 🎨
