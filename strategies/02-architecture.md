...
**Decision**: **Monorepo** (small, shallow, and intentional)

**Rationale**:

- Better separation of concerns (react components vs design tokens)
- Easier to add new packages later (CLI, docs, other frameworks)
- Modern tooling (pnpm workspaces) makes it simple
...
tinybigui/
├── packages/
│   ├── react/              # @tinybigui/react - Main component library
│   │   ├── src/
│   │   ├── package.json
│   │   └── tsconfig.json
│   │
│   └── tokens/             # @tinybigui/tokens - Design tokens
│       ├── src/
│       ├── package.json
│       └── tsconfig.json
│
...
### React Package Directory Structure
...
├── styles.css                            # Library stylesheet (includes tokens)
...
```javascript
// Tailwind v4 is CSS-first — we map design tokens in @tinybigui/tokens/tokens.css
// using CSS variables + @theme / @utility directives where needed.
//
// Apps consume tokens by importing:
...
Our monorepo structure allows easy addition of:

```
packages/
├── react/          # ✅ @tinybigui/react - Main library
├── tokens/         # ✅ @tinybigui/tokens - Design tokens
├── cli/            # 🔮 @tinybigui/cli - CLI tools (future)
├── vue/            # 🔮 @tinybigui/vue - Vue components (future)
├── svelte/         # 🔮 @tinybigui/svelte - Svelte components (future)
└── docs/           # 🔮 Documentation site (future)
```

We start with `react/` and `tokens/` only.
